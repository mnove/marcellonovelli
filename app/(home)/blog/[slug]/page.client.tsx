...
import { useHeadroom, useInViewport, useScrollSpy } from "@mantine/hooks";
    ...
    <>
      <>
        <motion.div
          ...
          className={`container flex w-full flex-row items-center gap-2 border bg-linear-to-r from-emerald-800 to-teal-700 py-3 md:rounded-xl md:px-4`}
        >
          <div>
            {" "}
            <Button
        ...
        <motion.div
          ...
          className={`container flex w-full flex-row items-center gap-2 border py-3 md:rounded-xl md:px-4 ${
            inViewport ? "opacity-0" : "opacity-100"
          } bg-linear-to-r from-emerald-800 to-teal-700`}
        >
          <div>
            {" "}
            <Button
...
export const BlogTOC = () => {
  ...
  const headings = data.map((heading, index) => (
    <div key={heading.id} className="relative hidden lg:block">
      <div
        className={`bg-primary absolute h-full w-[2px] transition-all duration-500 ${
      ...
      <div
        ...
        className={`cursor-pointer border-l py-1 text-sm ${
